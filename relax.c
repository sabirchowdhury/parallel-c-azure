...
void generateMatrix(int size, double* matrix);

// Helper to display the matrix given column width and number of elements
void displayMatrix(double* matrix, int columns, int nElements);
...
int main(int argc, char **argv) {
  ...
  if(rank == 0) {
    ...
    matrix=calloc((unsigned int) (nColumns*nColumns), sizeof(double));
  ...
  // matrix
  double* subsection=malloc(sizeof(double) * (unsigned int) elementsInProc);
  // newSubsection is the resultant array where calculations are stored
  double* newSubsection=malloc(sizeof(double) * (unsigned int) elementsInProc);
  // double pointer created as a temporary medium to swap two pointers
  double* pointerSwap;
  ...
  MPI_Scatterv(matrix, sendElementsPerProc, sendDisplacement, MPI_DOUBLE, 
                subsection, elementsInProc, MPI_DOUBLE, 0, MPI_COMM_WORLD);
  ...
  while (globalPrecision > 0) {
    ...
    double* topBorderRow = newSubsection;
    double* topRow = newSubsection + nColumns;
    double* bottomRow = newSubsection + elementsInProc - 2 * nColumns;
    double* bottomBorderRow = newSubsection + bottomBorderPosition;
  ...
  double* processorEditZoneStart = newSubsection + nColumns;
  MPI_Gatherv(processorEditZoneStart, nElementsToEdit, MPI_DOUBLE,
              matrix, recvElementsPerProc, recvDisplacement, MPI_DOUBLE,
                                                      0, MPI_COMM_WORLD);
...
void generateMatrix(int size, double* matrix){
    for (int x = 0; x < size; x++) {
        matrix[x] = 1.0;                  // top border
        matrix[x + (size-1)*size] = 1.0;  // bottom border
        matrix[size * x] = 1.0;            // left border
        matrix[size-1 + size * x] = 1.0;  // right border
    }
}
