...
void generateMatrix(int size, float* matrix){
    for (int x = 0; x < size; x++) {
        matrix[x] = rand()%100;                  // top border
        matrix[x + (size-1)*size] = rand()%100;  // bottom border
        matrix[size * x] = rand()%100;            // left border
        matrix[size-1 + size * x] = rand()%100;  // right border
    }
}
...
void testAllocateRows(int sendDisplacement,
  ...
  int sendYEnd = (int) floor((sendDisplacement + sendElementsPerProc) / nColumns);
  ...
  int recvYEnd = (int) floor((recvDisplacement + recvElementsPerProc) / nColumns);
